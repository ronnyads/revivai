    ...
    if (type === 'image') {
      ...
      if (talkingMode === 'exact_speech') {
        ...
        if (!hasExternalAudio && !autoGeneratedVoiceUrl) {
          const speechForTts = String(normalizedInputParams.speech_text_chunk ?? normalizedInputParams.speech_text_normalized ?? normalizedInputParams.speech_text_raw ?? '')
          if (speechForTts) {
            autoGeneratedVoiceUrl = await generateVoiceGrok({
              script: speechForTts,
              voice_id: String(normalizedInputParams.voice_id ?? 'ara'),
              assetId: asset.id,
              userId,
            })
