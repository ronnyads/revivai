    ...
    if (type === 'image') {
      ...
      if (talkingMode === 'exact_speech') {
        ...
        if (!hasExternalAudio && !autoGeneratedVoiceUrl) {
          ...
          // ElevenLabs IDs têm 15+ chars alfanuméricos — usar ElevenLabs TTS para eles
          const isElevenLabsVoiceId = /^[A-Za-z0-9]{15,}$/.test(rawVoiceId)
          try {
            if (isElevenLabsVoiceId) {
              autoGeneratedVoiceUrl = await generateVoiceGoogle({
                script: speechForTts,
                voice_id: rawVoiceId,
                speed: Number(normalizedInputParams.speed ?? 1.0),
                assetId: asset.id,
            ...
            console.error('[talking_video] TTS falhou:', ttsMsg)
            throw Object.assign(
              new Error(`Falha ao gerar áudio (TTS): ${ttsMsg}`),
