...
/* ─────────────────────────────────────────────────────────────────────────────
   ...
   Phase 2: Colorizes an already-restored photo. Costs 1 credit.
───────────────────────────────────────────────────────────────────────────── */
export async function POST(req: NextRequest) {
  ...
  // ── Fetch photo (must belong to user and be done) ──
  ...
  // Source: use colorization_url if already colorized before, else use restored_url
  const sourceUrl = photo.restored_url

  try {
    if (!process.env.REPLICATE_API_TOKEN) throw new Error('REPLICATE_API_TOKEN não configurada')
    const replicate = new Replicate({ auth: process.env.REPLICATE_API_TOKEN })

    // ── Run DDColor synchronously (fast model ~1-3s) ──
    ...
    while (result.status !== 'succeeded' && result.status !== 'failed' && result.status !== 'canceled') {
      if (Date.now() - startTime > 60000) throw new Error('Timeout na colorização')
      await new Promise(r => setTimeout(r, 2000))
      result = await replicate.predictions.get(result.id)
    }

    if (result.status !== 'succeeded' || !result.output) {
      throw new Error(result.error ? String(result.error) : 'DDColor falhou')
    }
    ...
    if (!colorizedUrl) throw new Error('DDColor retornou saída inválida')

    // ── Save result and debit credit ──
    const admin = createAdminClient()
    await admin.from('photos').update({ colorization_url: colorizedUrl }).eq('id', photoId)
    await admin.rpc('debit_credit', { user_id_param: user.id })

    console.log(`[colorize] Photo ${photoId} colorized: ${colorizedUrl}`)
    return NextResponse.json({ colorization_url: colorizedUrl })

  } catch (err: any) {
    console.error('[colorize] Error:', err.message)
    return NextResponse.json({ error: `Erro na colorização: ${err.message}` }, { status: 500 })
  }
}
