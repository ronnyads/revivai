    ...
    <div className="flex flex-col border-t border-white/5 max-w-5xl mx-auto">
      {items.map((item, i) => (
        <div key={i} className="border-b border-white/5 bg-transparent hover:bg-white/[0.01] transition-all duration-500">
          ...
          >
            <span className={`text-base md:text-lg font-bold uppercase tracking-tight transition-all duration-700 ${open === i ? 'text-[#54D6F6]' : 'text-white/80 group-hover:text-white'}`}>
              {item.question}
            </span>
            <div className={`shrink-0 flex items-center justify-center w-10 h-10 border transition-all duration-700 ${open === i ? 'border-[#54D6F6] bg-[#00ADCC]/10 rotate-180' : 'border-white/10 group-hover:border-white/20'}`}>
               {open === i ? <Minus size={16} className="text-[#54D6F6]" /> : <Plus size={16} className="text-white/40" />}
            </div>
          </button>
